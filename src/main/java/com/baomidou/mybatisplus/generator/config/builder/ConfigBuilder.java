...
public class ConfigBuilder {
    ...
    public ConfigBuilder(PackageConfig packageConfig, DataSourceConfig dataSourceConfig, StrategyConfig strategyConfig,
        ...
        if (null == packageConfig) {
        	if(StringUtils.isEmpty(this.globalConfig.getModelOutDir())) {
        		this.globalConfig.setModelOutDir(this.globalConfig.getOutputDir());
        	}
            handlerPackage(this.template, this.globalConfig.getOutputDir(),this.globalConfig.getModelOutDir(), new PackageConfig());
        } else {
            handlerPackage(this.template, this.globalConfig.getOutputDir(),this.globalConfig.getModelOutDir(), packageConfig);
        }
     ...
     */
    private void handlerPackage(TemplateConfig template, String outputDir, String modeloutputDir,PackageConfig config) {
        ...
        if (StringUtils.isNotEmpty(template.getModel(getGlobalConfig().isKotlin()))) {
            pathInfo.put(ConstVal.MODEL_PATH, joinPath(modeloutputDir+"//src//main//java", packageInfo.get(ConstVal.MODEL)));
        }
        if (StringUtils.isNotEmpty(template.getEntity(getGlobalConfig().isKotlin()))) {
            pathInfo.put(ConstVal.ENTITY_PATH, joinPath(outputDir+"//src//main//java", packageInfo.get(ConstVal.ENTITY)));
        }
        if (StringUtils.isNotEmpty(template.getMapper())) {
            pathInfo.put(ConstVal.MAPPER_PATH, joinPath(outputDir+"//src//main//java", packageInfo.get(ConstVal.MAPPER)));
        }
        if (StringUtils.isNotEmpty(template.getXml())) {
            pathInfo.put(ConstVal.XML_PATH, joinPath(outputDir+"//src//main//resources//mybatis//mapper", packageInfo.get(ConstVal.XML)));
        }
        if (StringUtils.isNotEmpty(template.getService())) {
            pathInfo.put(ConstVal.SERIVCE_PATH, joinPath(outputDir+"//src//main//java", packageInfo.get(ConstVal.SERIVCE)));
        }
        if (StringUtils.isNotEmpty(template.getServiceImpl())) {
            pathInfo.put(ConstVal.SERVICEIMPL_PATH, joinPath(outputDir+"//src//main//java", packageInfo.get(ConstVal.SERVICEIMPL)));
        }
        if (StringUtils.isNotEmpty(template.getController())) {
            pathInfo.put(ConstVal.CONTROLLER_PATH, joinPath(outputDir+"//src//main//java", packageInfo.get(ConstVal.CONTROLLER)));
        }
    }
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            if(StringUtils.isEmpty(globalConfig.getModelName())) {
            	globalConfig.setModelName(tableInfo.getEntityName());
            }
            if (StringUtils.isNotEmpty(globalConfig.getModelName())) {
                tableInfo.setXmlName(String.format(globalConfig.getModelName(), tableInfo.getModelName()));
            } else {
                tableInfo.setXmlName(tableInfo.getEntityName() + ConstVal.MODEL);
            }
            ...
            if(StringUtils.isEmpty(tableInfo.getApiName())) {
            	tableInfo.setApiName(tableInfo.getEntityName()+"API");
            }
            //强制开启字段注解
    ...
    private List<TableInfo> getTablesInfo(StrategyConfig config) {
        ...
        //需要反向生成或排除的表信息
        List<TableInfo> includeTableList = new ArrayList<TableInfo>();
        List<TableInfo> excludeTableList = new ArrayList<TableInfo>();
    ...
    /**
     * 
     * getSuperModelClass:对外的model路径，和dto等价. <br/> 
     * 
     * @author l2iu 
     * @return 
     * @since JDK 1.6
	...
	public String getSuperAPIClass() {
		return superAPIClass;
	}

	public void setSuperAPIClass(String superAPIClass) {
		this.superAPIClass = superAPIClass;
	}
